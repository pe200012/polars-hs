...
foreign import ccall safe "phs_dataframe_to_dummies"
    phs_dataframe_to_dummies ::
        Ptr RawDataFrame ->
        CBool ->
        Ptr CString ->
...
foreign import ccall safe "phs_dataframe_with_row_index"
    phs_dataframe_with_row_index :: Ptr RawDataFrame -> CString -> CBool -> Word64 -> Ptr (Ptr RawDataFrame) -> Ptr (Ptr RawError) -> IO CInt
...
foreign import ccall safe "phs_dataframe_shift"
    phs_dataframe_shift :: Ptr RawDataFrame -> CLLong -> Ptr (Ptr RawDataFrame) -> Ptr (Ptr RawError) -> IO CInt
...
foreign import ccall unsafe "phs_lazyframe_gather_every"
    phs_lazyframe_gather_every :: Ptr RawLazyFrame -> Word64 -> Word64 -> Ptr (Ptr RawLazyFrame) -> Ptr (Ptr RawError) -> IO CInt

foreign import ccall unsafe "phs_lazyframe_unpivot"
    phs_lazyframe_unpivot ::
        Ptr RawLazyFrame ->
        CBool ->
        Ptr CString ->
...
foreign import ccall unsafe "phs_lazyframe_join_ex"
    phs_lazyframe_join_ex ::
        Ptr RawLazyFrame ->
        Ptr RawLazyFrame ->
        Ptr (Ptr RawExpr) ->
...
foreign import ccall unsafe "phs_lazyframe_join_asof"
    phs_lazyframe_join_asof ::
        Ptr RawLazyFrame ->
        Ptr RawLazyFrame ->
        Ptr RawExpr ->
...
foreign import ccall safe "phs_series_clear"
    phs_series_clear :: Ptr RawSeries -> Ptr (Ptr RawSeries) -> Ptr (Ptr RawError) -> IO CInt

foreign import ccall safe "phs_series_new_from_index"
    phs_series_new_from_index :: Ptr RawSeries -> Word64 -> Word64 -> Ptr (Ptr RawSeries) -> Ptr (Ptr RawError) -> IO CInt
